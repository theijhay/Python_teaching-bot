...
class ActionProvideDecorators(Action):
    def name(self) -> Text:
        return "action_provide_decorators"

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        decorators_info = ("Decorators are functions that modify the behavior of other functions or methods. For example:\n"
                           "```python\n"
                           ...
                           "    print('Hello!')\n"
                           "```")
        dispatcher.utter_message(text=decorators_info)
        return []

class ActionProvideGenerators(Action):
    def name(self) -> Text:
        return "action_provide_generators"

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        generators_info = ("Generators are functions that return an iterable set of items. They are defined using the `yield` keyword. For example:\n"
                           "```python\n"
                           "def my_generator():\n"
                           "    yield 1\n"
                           "    yield 2\n"
                           "    yield 3\n"
                           "```")
        dispatcher.utter_message(text=generators_info)
        return []

class ActionProvideLambda(Action):
    def name(self) -> Text:
        return "action_provide_lambda"

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        lambda_info = ("Lambda functions are small anonymous functions defined using the `lambda` keyword. For example:\n"
                       "```python\n"
                       "add = lambda x, y: x + y\n"
                       "print(add(2, 3))\n"
                       "```")
        dispatcher.utter_message(text=lambda_info)
        return []

class ActionProvideComprehensions(Action):
    def name(self) -> Text:
        return "action_provide_comprehensions"

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        comprehensions_info = ("Comprehensions provide a concise way to create lists, dictionaries, and sets. For example, a list comprehension:\n"
                               "```python\n"
                               "squares = [x**2 for x in range(10)]\n"
                               "```")
        dispatcher.utter_message(text=comprehensions_info)
        return []
...
class ActionProvideAdvancedTopics(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        advanced_topics_info = ("Advanced Python topics include metaclasses, decorators, generators, context managers, and asynchronous programming.")
        dispatcher.utter_message(text=advanced_topics_info)
        return []
